function Extract-ExcelTextAndNumberColumnsToNewWorkbook {
    [CmdletBinding()]
    param(
    ...
    $index = 2
    foreach ($value in $ColumnXNumber) {
    if ($value -ne $null -and $ColumnYNumber[$index-2] -ne $null -and $ColumnXNumber[$index-2] -ne "100.000%") {
        $newWorksheet.Cells.Item($index,1).Value2 = $value
        $newWorksheet.Cells.Item($index,2).Value2 = $ColumnYNumber[$index-2]
        $index++
    }
    }
    $index--
